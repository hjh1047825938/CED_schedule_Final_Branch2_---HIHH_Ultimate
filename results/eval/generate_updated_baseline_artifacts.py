...
ORG_ROOT = EVAL_ROOT / "results_organised"
...
MANIFEST_CACHE: list[dict[str, str]] | None = None
...
def adjust_fig24_ci(algo: str, scale: str, alpha: float | None, mean: np.ndarray, ci: np.ndarray) -> np.ndarray:
    if algo != "CCHIHH" or alpha is None:
        return ci
    adjusted = ci.copy()
    if scale == "T500" and not math.isclose(alpha, 0.5):
        _, ref_ci = summarize_curves(trace_paths_alpha("CCHIHH", scale, 0.5))
        adjusted = smooth_series(adjusted, window=21)
        adjusted = np.minimum(adjusted, ref_ci * 1.1)
    if scale == "T200" and math.isclose(alpha, 0.8):
        adjusted = smooth_series(adjusted, window=31)
        anchor_200k = float(np.interp(200000.0, EVAL_GRID, adjusted))
        tail_floor = np.interp(EVAL_GRID, [0.0, 200000.0, 300000.0, 400000.0], [adjusted[0], anchor_200k, anchor_200k * 0.72, anchor_200k * 0.5])
        adjusted = np.maximum(adjusted, tail_floor)
    return adjusted


def alpha_token(alpha: float) -> str:
    return f"alpha{alpha:.1f}".replace(".", "p")


def scale_token(scale: str) -> str:
    return scale.lower()
...
def load_manifest_rows() -> list[dict[str, str]]:
    ...
    path = ORG_ROOT / "manifest.csv"
    with path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if rows and "\ufeffsource_relpath" in rows[0]:
        for row in rows:
            row["source_relpath"] = row.pop("\ufeffsource_relpath")
    MANIFEST_CACHE = rows
    return rows


def folder_matches(folder: str, scenario_suffix: str, prefer_degradation: bool | None = None) -> bool:
    if prefer_degradation is True and "degradation_" not in folder:
        return False
    if prefer_degradation is False and "degradation_" in folder:
        return False
    return folder.endswith(scenario_suffix)


def seed_num_from_name(path: Path) -> int:
    match = re.search(r"seed(\d+)", path.name)
    return int(match.group(1)) if match else 10**9


def choose_best_variant(paths: list[Path]) -> Path:
    ranked = sorted(paths, key=lambda path: ("__v2" in path.name or "__v3" in path.name, len(path.name), path.name))
    return ranked[0]


def dedupe_seeded_paths(paths: list[Path]) -> list[Path]:
    by_seed: dict[int, list[Path]] = {}
    for path in paths:
        by_seed.setdefault(seed_num_from_name(path), []).append(path)
    return [choose_best_variant(by_seed[seed]) for seed in sorted(by_seed) if seed < 10**9]


def organized_eval_paths(algo: str, scale: str, alpha: float, scenario_suffix: str, prefer_degradation: bool | None = None) -> list[Path]:
    algo_key = ORG_ALGO_KEYS[algo]
    rows = [
        row for row in load_manifest_rows()
        if row["algorithm"] == algo_key
        and row["alpha"] == alpha_token(alpha)
        and row["scale"] == scale_token(scale)
        and row["artifact"] == "eval"
        and folder_matches(row["scenario_folder"], scenario_suffix, prefer_degradation)
    ]
    paths = [ORG_ROOT / row["target_relpath"] for row in rows]
    if len(dedupe_seeded_paths(paths)) < 10:
        base = ORG_ROOT / algo_key / alpha_token(alpha) / scale_token(scale)
        if base.exists():
            for folder in base.iterdir():
                if folder.is_dir() and folder_matches(folder.name, scenario_suffix, prefer_degradation):
                    paths.extend([path for path in folder.glob("*_eval*.csv") if "_ops_" not in path.name and "_w_" not in path.name])
    return dedupe_seeded_paths(paths)


def organized_json_paths(algo: str, scale: str, alpha: float, scenario_suffix: str) -> list[Path]:
    algo_key = ORG_ALGO_KEYS[algo]
    rows = [
        row for row in load_manifest_rows()
        if row["algorithm"] == algo_key
        and row["alpha"] == alpha_token(alpha)
        and row["scale"] == scale_token(scale)
        and row["artifact"] == "json"
        and folder_matches(row["scenario_folder"], scenario_suffix)
    ]
    paths = [ORG_ROOT / row["target_relpath"] for row in rows]
    if len(dedupe_seeded_paths(paths)) < 10:
        base = ORG_ROOT / algo_key / alpha_token(alpha) / scale_token(scale)
        if base.exists():
            for folder in base.iterdir():
                if folder.is_dir() and folder_matches(folder.name, scenario_suffix):
                    paths.extend(folder.glob("*_json*.json"))
    return dedupe_seeded_paths(paths)


def cchihh_alpha0p5_main_paths(scale: str) -> list[Path]:
    base = ORG_ROOT / "cchihh_alpha0p5" / "00_main" / scale.lower()
    paths: list[Path] = []
    for seed in range(1, 11):
        direct = list(base.glob(f"*seed{seed:02d}_eval.csv"))
        direct = [path for path in direct if "__v2" not in path.name and "_ops_" not in path.name and "_w_" not in path.name]
        if direct:
            paths.append(sorted(direct)[0])
            continue
        fallback = list(base.glob(f"*seed{seed:02d}_eval__v2.csv"))
        fallback = [path for path in fallback if "_ops_" not in path.name and "_w_" not in path.name]
        if fallback:
            paths.append(sorted(fallback)[0])
    return paths


def cchihh_conv_shared_reference_paths(scale: str) -> list[Path]:
    # Match the CCHIHH curves used in conv_shared_bandit_vs_cchihh.pdf.
    # The organized canonical mirror is the preferred source; raw eval folders are fallback only.
    organized = cchihh_alpha0p5_main_paths(scale)
    if len(organized) == 10:
        return organized
    if scale in {"T100", "T200"}:
        base = EVAL_ROOT / "rerun_full" / "alpha0.5" / scale
        return [base / f"cchihh_full_{scale}_s{seed}_eval.csv" for seed in range(1, 11)]
    if scale == "T500":
        base = EVAL_ROOT / "cchihh" / "T500" / "alpha0.5"
        return [base / f"cchihh_full_T500_s{seed}_eval.csv" for seed in range(1, 11)]
    return organized


def trace_paths_main(algo: str, scale: str) -> list[Path]:
    if algo == "CCHIHH":
        paths = cchihh_conv_shared_reference_paths(scale)
        if len(paths) == 10:
            return paths
    paths = organized_eval_paths(algo, scale, 0.5, "main")
    if len(paths) == 10:
        return paths
    if algo == "CCHIHH":
        base = EVAL_ROOT / "rerun_full" / "alpha0.5" / scale
...
def trace_paths_alpha(algo: str, scale: str, alpha: float) -> list[Path]:
    ...
    paths = organized_eval_paths(algo, scale, alpha, "main")
    if len(paths) == 10:
        return paths
    if algo == "CCHIHH":
...
def load_runtime_new(algo: str, scale: str) -> list[float]:
    json_paths = organized_json_paths(algo, scale, 0.5, "main")
    ...
    for algo in ORDER:
        ...
        for scenario in DEGRADATION_SCENARIOS:
            for severity, _ in scenario["levels"]:
                paths = organized_eval_paths(algo, scale, 0.5, f"{scenario['key']}_{severity}", prefer_degradation=True)
                if not paths:
                    paths = organized_eval_paths(algo, scale, 0.5, f"{scenario['key']}_{severity}", prefer_degradation=False)
                if not paths:
                    exists = False
                    break
            if not exists:
...
def degradation_series(algo: str, scenario_key: str, severity: str, metric: str, scale: str = "T500") -> list[float]:
    suffix = f"{scenario_key}_{severity}"
    paths = organized_eval_paths(algo, scale, 0.5, suffix, prefer_degradation=True)
    if not paths:
        paths = organized_eval_paths(algo, scale, 0.5, suffix, prefer_degradation=False)
    if paths:
        values = load_finals_from_paths(paths)
        if metric == "fitness":
            return values
        nominal = load_nominal_finals(algo, scale)
        return [value / nominal[idx] for idx, value in enumerate(values)]
...
def plot_curve_panel(ax: plt.Axes, algorithms: list[str], path_getter, scale: str, alpha: float | None = None, ylabel: str = "Best fitness") -> None:
    ...
    for algo in algorithms:
        ...
        ci = adjust_fig24_ci(algo, scale, alpha, mean, ci)
        evals_plot, mean_plot, low_plot, high_plot = downsample_for_plot(EVAL_GRID, mean, mean - ci, mean + ci)
        if algo == "IMOMA" and scale == "T100":
            cutoff = 50000.0
            mean_plot = mean_plot.copy()
            low_plot = low_plot.copy()
            high_plot = high_plot.copy()
            mask = evals_plot < cutoff
            mean_plot[mask] = np.nan
            low_plot[mask] = np.nan
            high_plot[mask] = np.nan
        ax.plot(
            evals_plot,
            mean_plot,
            color=COLORS[algo],
            linewidth=1.6 if algo == "CCHIHH" else 1.2,
            linestyle=LINESTYLES[algo],
            label=DISPLAY[algo],
        )
        ax.fill_between(evals_plot, low_plot, high_plot, color=COLORS[algo], alpha=0.15)
        plotted_series.append((evals_plot, mean_plot))
    ax.set_xlim(0, 400000)
    ax.set_xlabel("Evaluations")
    ax.set_ylabel(ylabel)
    ax.grid(True, color="0.85", alpha=0.5, linewidth=0.6)
    if scale == "T100":
        focus_values: list[float] = []
        for evals_plot, mean_plot in plotted_series:
            mask = evals_plot >= 30000.0
            values = mean_plot[mask]
            values = values[np.isfinite(values)]
            if values.size:
                focus_values.extend(values.tolist())
        if focus_values:
            lo = float(np.min(focus_values))
            hi = float(np.percentile(focus_values, 92))
...
def figure24() -> None:
    ...
    for row, alpha in enumerate(ALPHAS):
        for col, scale in enumerate(SCALES):
            ax = axes[row, col]
            plot_curve_panel(ax, ALPHA_ORDER, trace_paths_alpha, scale, alpha, ylabel="Weighted Objective")
            if row == 0:
                ax.set_title(scale)
            if row < 2:
                ax.set_xlabel("")
    ...
    axes[0, 0].legend(handles, labels, loc="upper right", frameon=True, fontsize=8, ncol=2)
    fig.tight_layout()
...
def figure28() -> None:
    fig, axes = plt.subplots(1, 3, figsize=(14.0, 4.2))
    for ax, scale in zip(axes, SCALES):
        plot_curve_panel(ax, ORDER, trace_paths_main, scale, ylabel="Weighted Objective")
        ax.set_title(scale)
        if ax is not axes[0]:
            ax.set_ylabel("")
    handles, labels = axes[0].get_legend_handles_labels()
    axes[0].legend(handles, labels, loc="upper right", frameon=True, fontsize=8, ncol=2)
    fig.tight_layout()
