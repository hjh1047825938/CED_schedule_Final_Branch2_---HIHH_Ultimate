...
def build_tasks(root: Path, generations: int, log_every: int, data_file: str):
    exe = root / "build" / "Release" / "CED_Schedule.exe"
    data_dir = root / "data"
    results_dir = root / "results" / "gen10000_seed1_10"
    ...
    for seed in range(1, 11):
        for name, solver_args in solvers:
            if name == "CCHIHH_base":
                log_name = f"CCHIHH_base_seed{seed}.txt"
            elif name == "CCHIHH_stable":
                log_name = f"CCHIHH_stable_seed{seed}.txt"
            else:
...
def main():
    ...
    tasks = build_tasks(root, args.generations, args.log_every, args.data_file)
