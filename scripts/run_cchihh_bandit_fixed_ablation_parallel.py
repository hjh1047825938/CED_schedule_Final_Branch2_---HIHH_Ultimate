...
CONFIGS = [
    ("cfg1_bandit_on", ["--solver", "CCHIHH", "--stable"]),
    ("cfg2_bandit_off", ["--solver", "CCHIHH", "--stable", "--cchihh_random_ops"]),
    ("cfg3_fixed_ops", ["--solver", "CCHIHH", "--stable", "--cchihh_fixed_ops"]),
]
...
def build_tasks(
    ...
    for seed in range(1, 11):
        for cfg_name, cfg_args in CONFIGS:
            ...
            if cfg_name == "cfg1_bandit_on":
                op_stats = results_dir / f"CCHIHH_{cfg_name}_opstats_seed{seed}.csv"
                cmd.extend(
                    [
                        "--cchihh_op_stats",
                        str(op_stats),
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory to store logs and op-stats.",
    )
    ...
    tasks = build_tasks(
        ...
        resample_gate=args.resample_gate,
        cnum=args.cnum,
        enum=args.enum,
        dnum=args.dnum,
        tnum=args.tnum,
